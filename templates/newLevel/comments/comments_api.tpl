{if $can_comment == 1 AND !$is_logged_in}
    <label>
        <span class="title__icsi-css"><b>{sprintf(lang('Будь ласка, авторизуйтеся, щоб залишити коментар', 'newLevel'), site_url($modules.auth))}</b></span>
    </label>
{/if}
<div class="comment__icsi-css" id="comment__icsi-css">
        ...
        <div class="main-form-comments__icsi-css {if !$comments_arr}noComments{/if}">
            <div class="frame-comments__icsi-css layout-highlight">
                <div class="title_h2__icsi-css title-default">
                    <div class="title">{lang('Залишити коментар','newLevel')}</div>
                </div>
                        ...
                        <form method="post">
                            {if !$is_logged_in}
                                {if $use_moderation}
                                    <label>
                                        <span class="frame_form_field__icsi-css">
                                            <div class="msg">
                                                <div class="success">
                                                    {lang('Коментар буде відправлений на модерацію','newLevel')}
                                                </div>
                                            </div>
                                        </span>
                                    </label>
                                ...
                                <label style="width: 45%;float: left;">
                                    <span class="title__icsi-css">{lang('Ваше ім`я','newLevel')}</span>
                                    <span class="frame_form_field__icsi-css">
                            ...
                            {/if}
                            <label>
                                <span class="title__icsi-css">{lang('Текст коментаря:')}</span>
                                <span class="frame_form_field__icsi-css">
                            ...
                            <div class="frameLabel__icsi-css">
                                <span class="title__icsi-css f_l t-a_l">{lang('Ваша оцінка:', 'newLevel')}</span>
                            ...
                            {if $use_captcha}
                                <label>
                                    <span class="title__icsi-css">{lang('Код захисту')}</span>
                                    {$cap_image}
                                    <span class="frame_form_field__icsi-css">
                            ...
                            <div class="frameLabel__icsi-css">
                                <span class="frame_form_field__icsi-css">
                                    <div class="btn-form">
                                        <input type="submit" value="{lang('Коментувати')}" onclick="Comments.post
                                            (this)"/>
                                    </div>
                                </span>
                            </div>
    ...
    {if $comments_arr}
        <div class="frame-list-comment__icsi-css">
            <ul class="sub-1 product-comment patch-product-view">
                {foreach $comments_arr as $key => $comment}
                    <li>
                        ...
                        {if $comment.rate != 0}
                            <div class="mark-pr">
                                <span>{lang('Оцінка:','newLevel')}</span>
                        ...
                        <div class="frame-comment__icsi-css">
                            ...
                            {if $comment.text_plus != Null}
                                <p>
                                    <b>{lang('Так', 'newLevel')}</b><br>
                                    {$comment.text_plus}
                                </p>
                            {/if}
                            {if $comment.text_minus != Null}
                                <p>
                                    <b>{lang('Ні', 'newLevel')}</b><br>
                                    {$comment.text_minus}
                                </p>
                            {/if}
                        </div>
                        <div class="func-button-comment__icsi-css">
                            {if $can_comment == 0 OR $is_logged_in}
                                <div class="btn__icsi-css f_l__icsi-css">
                                    <button type="button" data-rel="cloneAddPaste" data-parid="{$comment['id']}">
                                        ...
                                        <span class="text-el d_l_1">{lang('Відповісти')}</span>
                                    </button>
                                </div>
                            {/if}
                            ...
                            <div class="f_r__icsi-css">
                                <span>
                                    <span class="s-t">{lang('Чи корисний відгук?','newLevel')}</span>
                                    <span class="btn__icsi-css like__icsi-css">
                                        <button type="button" class="usefullyes" data-comid="{echo $comment.id}">
                                            <span class="text-el">{lang('Так','newLevel')} <span class="yesholder{$comment.id}">({echo $comment.like})</span></span>
                                        </button>
                                    </span>
                                    <span class="btn__icsi-css dis-like__icsi-css">
                                        <button type="button" class="usefullno" data-comid="{echo $comment.id}">
                                            <span class="text-el">{lang('Ні','newLevel')} <span class="noholder{$comment.id}">({echo $comment.disslike})</span></span>
                                        </button>
                                    </span>
                                </span>
                            </div>
                            ...
                            <ul class="frame-list-comment__icsi-css sub-2">
                                {foreach $comment_ch as $com_ch}
                                    {if $com_ch.parent == $comment.id}
                                        <li>
                                            <div class="author-data-comment__icsi-css">
                                                <span class="s-t">{lang('Коментар від','newLevel')}</span>
                        ...
                        <div class="btn-all-comments">
                            <button type="button"><span class="text-el" data-hide='<span class="d_l_1">{lang('Приховати','newLevel')}</span> ↑' data-show='<span class="d_l_1">{lang('Смотреть все ответы','newLevel')}</span> ↓'></span></button>
                        </div>
                    </li>
                {/foreach}
            </ul>
            <button class="t-d_n f-s_0 s-all-d ref d_n_" data-trigger="[data-href='#comment']" data-scroll="true">
                ...
                <span class="text-el">{lang('Продивитися всі відповіді','newLevel')}</span>
            </button>
        </div>
    {/if}
    ...
    <div class="frame-drop-comment__icsi-css" data-rel="whoCloneAddPaste">
        <div class="form-comment__icsi-css form__icsi-css layout-highlight frame-comments__icsi-css">
            <div class="title_h2__icsi-css title-default">
                <div class="title">{lang('Ваша відповідь','newLevel')}</div>
            </div>
            <div class="inside-padd">
                <form>
                    ...
                    {if !$is_logged_in}
                        ...
                        <label>
                            <span class="frame_form_field__icsi-css">
                                <div class="msg">
                                    <div class="success">
                                        {lang('Коментар буде відправленний на модерацію','newLevel')}
                                    </div>
                                </div>
                            </span>
                        </label>
                    {/if}
                    <label>
                        <span class="title__icsi-css">{lang('Текст відповіді:','newLevel')}</span>
                        <span class="frame_form_field__icsi-css">
                    ...
                    <div class="frameLabel__icsi-css">
                        <span class="frame_form_field__icsi-css">
                            ...
                            <span class="btn-form">
                                <input type="submit" value="{lang('Коментувати', 'newLevel')}" onclick="Comments.post(this)"/>
                            </span>
                        </span>
                    </div>
                </form>
