...
<script type="text/template" id="cartPopupTemplate">
    {literal}
        ...
        <span class="text-el"><span class="f-s_14">←</span> {/literal}{lang('Повернутися до покупок','newLevel')}{literal}</span>
        </button>
        </div>
        <div class="btn-cart btn-cart-p f_r">
        <a href="/shop/cart">
        <span class="icon_cart_p"></span>
        <span class="text-el">{/literal}{lang('Оформити замовлення','newLevel')}{literal}</span>
        </a>
        </div>
        </div>
        </div>
