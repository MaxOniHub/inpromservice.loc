...
class BaseCart extends \ShopController {
    ...
    public function __construct() {
        ...
        try {
            ...
            foreach ($itemsArray as $itemData) {
                ...
                $this->items[] = new CartItem($itemData['instance'], $itemData['id'], $itemData['quantity'], $itemData['price']);
            }
        } catch (Exception $exc) {
    ...
    /**
     * get sum Kit Discount
     * @access private
     * @author DevImageCms
     * @return float $total
     * @copyright (c) 2013, ImageCMS
     */
    public function getDiscountKits(){
        
        $kits = $this->getItems('ShopKit');
        if (count($kits['data'])>0)
            foreach ($kits['data'] as $kit)
                $total += ($kit->originPrice - $kit->price)*$kit->quantity;
    ...
    public function recountTotalPrice() {
        ...
        \CMSFactory\Events::create()->registerEvent(array('object' => $this), 'Cart:Operation')->runFactory();
        
        if ($this->gift_value)
            $this->setTotalPrice($this->getTotalPrice() - $this->gift_value);
     ...
     * @copyright (c) 2013, ImageCMS
     */
    public function recountOriginTotalPrice() {
        $this->originPrice = 0;
