...
class BaseOrder extends \ShopController {
    ...
    public static function create($data = null, &$products = null) {
        ...
        if ($cart->gift_value)
            $cart->setTotalPrice($cart->getTotalPrice() - $cart->gift_value);
...
//        var_dump($cart->getTotalPrice());
        ...
        if ($disc = $cart->discount_info) {
            $order->setdiscount($disc['result_sum_discount']);
            if ($disc['sum_discount_no_product'] > $disc['sum_discount_product'])
                $order->setdiscountinfo(json_encode($disc));
        }
        /** Try to save order * */
        if ($order->save()) {
    ...
    public function getDeliveryMethods() {
        $deliveryMethods = \SDeliveryMethodsQuery::create()
                ...
                ->enabled()
                ->orderBy('SDeliveryMethodsI18n.Name')
                ->find();
