...
//            $model = $model->where('');
        ...
        if (isset(ShopCore::$_GET['number']) && ShopCore::$_GET['number'] != '') {
            $model = $model->where('ProductVariant.Number = ?', ShopCore::$_GET['number']);
        }
        ...
        if (isset(ShopCore::$_GET['min_price']) && ShopCore::$_GET['min_price'] > 0) {
            $model = $model->where('ProductVariant.Price >= ?', ShopCore::$_GET['min_price']);
        }

        if (isset(ShopCore::$_GET['max_price']) && ShopCore::$_GET['max_price'] > 0) {
        ...
        //$config['base_url'] = site_url('admin/components/run/shop/search/index/?'.http_build_query(ShopCore::$_GET));
