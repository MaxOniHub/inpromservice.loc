    ...
    <section class="mini-layout">
        <div class="frame_title clearfix">
            ...
            <div class="pull-right">
                <div class="d-i_b">
                    {if $_GET == null || ($_GET['_pjax']!=null && count($_GET ==1))}{/if}
                    <button title="{lang('Filter','admin')}" type="submit" class="btn btn-small action_on disabled listFilterSubmitButton"><i class="icon-filter"></i>{lang('Filter','admin')}</button>
                    <button title="{lang('Reset filter','admin')}" onclick="if(!$(this).hasClass('disabled')) location.href = '/admin/components/run/shop/search/index'{if $_GET['WithoutImages'] == 1}'?WithoutImages=1'{/if}"  type="button" class="btn btn-small action_on {if $_GET == null || ($_GET['_pjax']!=null && count($_GET ==1))}disabled {/if}"><i class="icon-refresh"></i>{lang('Cancel filter','admin')}</button>
            ...
            <table class="table table-striped table-bordered table-hover table-condensed products_table">
                ...
                <td class="number">
                    <label>
                        ...
                        <span class="o_h d_b"><input type="text" name="min_price" value="{$_GET.min_price}" maxlength="21"/></span>
                    </label>
                    <label>
                        <span class="pull-left"><span class="neigh_form_field help-inline"></span>{lang('To','admin')}&nbsp;&nbsp;</span>
                        <span class="o_h d_b"><input type="text" name="max_price" value="{$_GET.max_price}" maxlength="21"/></span>
                    </label>
                </td>
                {form_csrf()}
                </form>
