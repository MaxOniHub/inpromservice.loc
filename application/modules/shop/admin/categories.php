...
class ShopAdminCategories extends ShopAdminController {
    ...
    public function create() {
        ...
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $tree = ShopCore::app()->SCategoryTree->getTree(SCategoryTree::MODE_SINGLE);

                $category = $tree[$model->id];

                $category->createPaths($tree);
                $model->setFullPath($category['full_path']);
                $model->setFullPathIds($category['full_path_ids']);
                $model->save();
    ...
    public function delete() {
        ...
        } elseif ($models = SCategoryQuery::create()->findBy('Id', $category_id)) {
            foreach ($models as $c) {
                $c->delete();
            }
    ...
    private function printCategory($category) {
        ...
        if (is_object($category)) {
            ...
            $catToDisplay->url = $category['url'];
            $catToDisplay->active = $category['active'];
            $level = count(explode('/', $category['url']));
            $catToDisplay->level = $level;
