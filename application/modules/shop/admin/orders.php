...
//            $cvcv = array('SOrders.UserFullName LIKE ?' => '%' . ShopCore::$_GET['customer'] . '%', 'SOrders.UserEmail LIKE ?' => '%' . ShopCore::$_GET['customer'] . '%', 'SOrders.UserPhone LIKE ?'=> '%'.ShopCore::$_GET['customer'].'%');
        if (ShopCore::$_GET['customer'])
            $model = $model->_and()
                    ->where('SOrders.UserFullName LIKE ?', '%' . ShopCore::$_GET['customer'] . '%')
                    ->_or()
                    ->where('SOrders.UserEmail LIKE ?', '%' . ShopCore::$_GET['customer'] . '%')
                    ->_or()
                    ->where('SOrders.UserPhone LIKE ?', '%' . ShopCore::$_GET['customer'] . '%');
        ...
        // Count total orders
        if ((ShopCore::$_GET['orderMethod'] != '' && ShopCore::$_GET['orderCriteria'] != '' && method_exists($model, 'filterBy' . ShopCore::$_GET['orderMethod']) ) or ShopCore::$_GET['orderMethod'] == 'Id') {
            switch (ShopCore::$_GET['orderCriteria']) {
                case 'ASC':
                    $nextOrderCriteria = 'DESC';
                    $nOArr = '&darr;';
                    $model = $model->orderBy(ShopCore::$_GET['orderMethod'], Criteria::ASC);
                    break;

                case 'DESC':
                    $nextOrderCriteria = 'ASC';
                    $nOArr = '&uarr;';
                    $model = $model->orderBy(ShopCore::$_GET['orderMethod'], Criteria::DESC);
                    break;
            }
        }
        else
