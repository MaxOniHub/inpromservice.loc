...
//+++++++++++++++++++++++
    ...
    public function ajaxChangeActive($productId = null) {
        ...
        \CMSFactory\Events::create()->registerEvent(array(
            'model' => $model,
            'userId' => $this->dx_auth->get_user_id()), 'ShopAdminProducts:ajaxChangeActive');
        \CMSFactory\Events::runFactory();
    }
    ...
    public function ajaxDeleteProducts() {
        if (sizeof($_POST['ids'] > 0)) {
            ...
            \MediaManager\Image::create()->deleteImagebyProductId($imagesToDelete);
    ...
    /**
     * If product was cloned it's image can be used with two or more 
     * products. Method returns product ids whitch image is in
     * usage of another product
     * @return type
     */
    private function getPrIdsNotAllowedToDelete() {
        $query = "
                SELECT 
                    `product_id`,
                    `mainImage`
        ...
        if ($result->num_rows() > 0) {
            ...
            $all = array();
            $notAllowedToDelete = array();
            foreach ($result->result() as $row) {
                if (in_array($row->mainImage, $all)) {
                    $notAllowedToDelete[] = $row->product_id;
                }
                $all[] = $row->mainImage;
            }
        }
    ...
    private function getImagesToDelete() {
        $notDelete = $this->getPrIdsNotAllowedToDelete();
        $toDelete = array();
        foreach ($_POST['ids'] as $productId) {
            if (!in_array($productId, $notDelete)) {
                $toDelete[] = $productId;
            }
        }
        return $toDelete;
