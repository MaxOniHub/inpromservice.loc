...
class ShopAdminNotifications extends ShopAdminController {
    ...
    public function index($offset = 0, $orderField = '', $orderCriteria = '') {
        ...
        $config['base_url'] = $this->createUrl('notifications/index/' . ShopCore::$_GET['status'] . '/');
    ...
    public function edit($id) {
        ...
        if ($_POST) {
            ...
            $model->save();
            //$this->ajaxShopDiv('notifications/index?status='.ShopCore::$_GET['back_to']);
        } else {
    ...
    public function search($offset = 0, $orderField = '', $orderCriteria = '') {
        ...
        if (ShopCore::$_GET['user_email'])
            $model = $model->filterByUserEmail(ShopCore::$_GET['user_email']);

        if (ShopCore::$_GET['user_phone'])
            $model = $model->where('SNotifications.UserPhone LIKE "%' . encode(ShopCore::$_GET['user_phone']) . '%"');

        if (ShopCore::$_GET['created'])
            $model = $model->where('FROM_UNIXTIME(SNotifications.DateCreated, \'%Y-%M-%D\') < ?', ShopCore::$_GET['created']);

        if (ShopCore::$_GET['actual'])
