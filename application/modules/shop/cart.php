...
class Cart extends \Cart\BaseCart {
    ...
    public function __construct() {
        ...
        if (!$this->useDeprecated) {
            ...
            $this->baseCart->recountOriginTotalPrice();
            $this->baseCart->recountTotalPrice();
        }
    ...
    public function index() {
        ...
        if ($this->useDeprecated) {
            ...
            $c = new Criteria();
            $c->add(SPaymentMethodsPeer::ACTIVE, 1, Criteria::EQUAL);
            ...
            if ($this->baseCart->discount_info) {
    ...
    protected function _makeOrder() {
        ...
        foreach (ShopCore::app()->SCart->loadProducts() as $cartItem) {
            ...
            if ($cartItem['model'] instanceof SProducts) {
                ...
                $totalPrice += number_format($price, ShopCore::app()->SSettings->pricePrecision, '.', '') * $cartItem['quantity'];
...
//
        ...
        \CMSFactory\Events::create()->registerEvent(array('order' => $order, 'key' => $_POST['giftkey'], 'price' => $totalPrice), 'Cart:MakeOrder')->runFactory();
